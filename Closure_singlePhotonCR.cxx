...
void Closure_singlePhotonCR(){
  ...
  readConfigFile(config.data(), "crSel", srCut);
  readConfigFile(config.data(), "crpassID", srpassCut);
  readConfigFile(config.data(), "crfailID", srfailCut);
  ...
  map<TString, TH1F*> srfailTmpHists;

  TFile *f_FF = new TFile(Form("forSR_pass_%s.root", cfSuffix.data()), "read");
  ...
  for (auto it = samples_id.begin(); it != samples_id.end(); it++){
    ...
    TH1F h_sr_fail_tmp("h_sr_fail_tmp", "", nBins, varMin, varMax);
    h_sr_fail_tmp.Sumw2();

    if(sample!="yy2L"&&sample!="Sherpa2_diphoton"){
      auto df_sr_fail_x_FF = df.Filter(srfailAllCut);

      df_sr_fail_x_FF.Foreach([&h_FF, &h_sr_fail_x_FF_nom, &h_sr_fail_tmp](double var, double tau0_pt, double tau1_pt, double w){
        ...
        h_sr_fail_x_FF_nom.Fill(var/1000, w*ff1); // Closure Test
        h_sr_fail_tmp.Fill(var/1000, w);
      ...
      srfailTmpHists[sample] = (TH1F*)h_sr_fail_tmp.Clone(sample+"_SR_failID_tmp");
    }
  }

  cout<<"cr pass"<<endl;
  for(auto h : srpassHists){
    cout<<h.first<<" "<<h.second->Integral()<<endl;
  }
  cout<<"cr fail"<<endl;
  for(auto h : srfailHists){
    cout<<h.first<<" "<<h.second->Integral()<<endl;
  }
  cout<<"cr fail tmp"<<endl;
  for(auto h : srfailTmpHists){
    cout<<h.first<<" "<<h.second->Integral()<<endl;
  }
  ...
  for(auto h : srpassHists){
    ...
    if(h_name=="yy2L") continue; // exclude from others
  ...
  stackHist(srpassHists, obsVar, "Closure_CR", cfSuffix, false, ignoreList);
